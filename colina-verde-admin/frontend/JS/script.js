...
function showTab(tab) {
  ...
  if (tab === "buffet") {
    ...
    const horarioContainer = document.getElementById(
      "horario_buffet_container"
    );
    ...
    dateInput.addEventListener("input", function () {
      ...
      if (val) {
        ...
        if (day === 1 || day === 2) {
          ...
          document.getElementById("buffetMsg").innerText =
            "Buffet não pode ser marcado para segunda ou terça-feira.";
          return;
        } else {
    ...
    async function verificarDataBuffet(data, horario) {
      ...
      const count = buffets.filter(
        (b) => b.data_buffet === data && b.horario_buffet === horario
      ).length;
      return count >= 1;
    }
    ...
    document.getElementById("criarBuffet").onclick = async function () {
      ...
      if (((day >= 3 && day <= 6) || day === 0) && !horario) {
        document.getElementById("buffetMsg").innerText =
          "Por favor, digite ou selecione o horário do buffet.";
        return;
      }
      ...
      if (duplicado) {
        document.getElementById(
          "buffetMsg"
        ).innerText = `Já existe buffet para esta data (${data}) e horário!`;
        return;
      }
    ...
    document.getElementById("atualizarBuffet").onclick = async function () {
      ...
      const res = await fetch(`http://localhost:3000/api/buffet/data/${data}`, {
        method: "PUT",
        body: formData,
      });
    ...
    document.getElementById("deletarBuffet").onclick = async function () {
      const dataInput = document.getElementById("data_buffet").value;
      if (!dataInput) {
        document.getElementById("buffetMsg").innerText =
          "Insira uma data válida para deletar.";
        return;
      }

      const res = await fetch(`http://localhost:3000/api/buffet/data/${dataInput}`, {
        method: "DELETE",
      });
      document.getElementById("buffetMsg").innerText = res.ok
        ? "Buffet deletado!"
        : "Erro ao deletar.";
    };
  }
  if (tab === "porcoes") {
    ...
    document.getElementById("atualizarPorcao").onclick = async function () {
      ...
      const res = await fetch(
        `http://localhost:3000/api/porcoes/nome/${encodeURIComponent(nome)}`,
        {
          method: "PUT",
          body: formData,
        }
    ...
    document.getElementById("deletarPorcao").onclick = async function () {
      ...
      document.getElementById("porcaoMsg").innerText = res.ok
        ? "Porção deletada!"
        : "Erro ao deletar.";
    };
  }
  if (tab === "drinks") {
    document.getElementById("criarDrink").onclick = async function () {
      ...
      document.getElementById("drinkMsg").innerText = res.ok
        ? "Drink criado!"
        : "Erro ao criar.";
    ...
    document.getElementById("atualizarDrink").onclick = async function () {
      ...
      const formData = new FormData(form);
      const res = await fetch(
        `http://localhost:3000/api/drinks/nome/${encodeURIComponent(nome)}`,
        {
          method: "PUT",
          body: formData,
        }
