...
async function fetchCardapio() {
        ...
        buffetList.innerHTML = buffetItens.map(item => `
            <div class="buffet-block" style="margin-bottom:32px;padding-bottom:16px;border-bottom:1px solid #e0e0e0;">
                ...
                <div style="flex:1;min-width:180px;">
                    ...
                    ${item.horario_buffet ? `<div style="color:#555;">Horário: ${formatBuffetTime(item.horario_buffet)}</div>` : ''}
                </div>
            </div>
        `).join("");
    ...
    if (porcoesContainer && Array.isArray(porcoes)) {
        porcoesContainer.innerHTML = porcoes.map(item => {
            let preco = item.preco_meia ? `R$ ${Number(item.preco_inteira).toFixed(2).replace('.', ',')} / R$ ${Number(item.preco_meia).toFixed(2).replace('.', ',')}` : `R$ ${Number(item.preco_inteira).toFixed(2).replace('.', ',')}`;
